...
import { SignForClaimingProps, SignForClaimingResult } from '../types/types';
 ...
 * @returns {Promise<SignForClaimingResult>} A promise that resolves to the result of the signing operation.
 *
...
export const signForReward = async ({
  ...
  rewardId,
  network,
}: SignForClaimingProps): Promise<SignForClaimingResult> => {
  try {
